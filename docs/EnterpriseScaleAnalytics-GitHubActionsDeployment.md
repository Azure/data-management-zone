...
| purviewRootCollectionAdminObjectIds | Specifies the list of user object IDs that are assigned as collection admin to the root collection in Purview. `az ad user list --display-name "{user name}" --query "[].objectId"`| [`{object-id-1}`, `{object-id-2}`] |
